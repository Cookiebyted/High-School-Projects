...
local function onTouch (target, event)
	...
	if event.phase == "began" then
		...
		-- check for the ended phase of the touch
...
function  Controls:drawControls()
	...
	self.control.arrowRect = display.newRoundedRect(self.control, 0,0, 30, 30, 6)
	self.control.arrowRect.strokeWidth = 2
	self.control.arrowRect:setFillColor( 0, 0, 0, 0.1 )
	self.control.arrowRect.image = display.newImageRect(self.control, self.image, 20, 20)
...
function Controls:desconstuctor()

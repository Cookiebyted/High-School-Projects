...
local function displayMan(textField)
	...
	local yourCharacterText = display.newText( "Your Character", centerX, centerY - 450, native.systemFont, 36 )
		...
		group:insert(yourCharacterText)
...
-- Function to display a "Finish" button, validate textField.text, remove display group and call displayMan function 
local function selectButton(group, textField)
	-- Display a button that when pressed will:
		-- Validate the text fields to make sure the text is between 1-100 and only contains numbers
			-- If 1-100 and only contains numbers:
	...
	-- Receives:
		...
		-- a reference to text field so they can be validated

	local function strengthHandleButtonEvent(event)
		-- Do data validation on (textField.text)
		-- If the text is not correct then display an alert
		...
		elseif tonumber(textField.text) > 100 then
			local alert = native.showAlert( "Error", "Please Enter Numbers Between 1-100", {"OK"} )
		-- Check textField.text is under 1
			-- If under 1, show alert
		elseif tonumber(textField.text) < 1 then
			local alert = native.showAlert( "Error", "Must Be A Number Between 1-100", {"OK"} )
		-- If textField is 1-100 and only contains numbers then
			-- Remove the first scene group and display function displayMan(textField)
		else
	...
	-- Align the button on the screen and insert it ino the right group
...
-- Create the textField variable from createTextBox and call selectButton
local function mainLoop ()
	-- Create a group to put all your objects in
	-- Create tables for all the clothing items
	-- Call function createClothesPicker(group, faceTable, hairTable, pantsTable, shirtTable, shoesTable) that creates the pickers and reset button
	-- Call function createTextBox(group) and store it in variable textField
	-- Call function selectButton(group, textField) to create the "Finish" Button, validate textField.text and remove display group/call displayMan function

	local group = display.newGroup()
	
	-- Create tables with all the options in them
